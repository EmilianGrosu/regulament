
const prs = {
    "Regulament": `
    <h1>TERMENI</h1>
    <h2>[IC] - In Character</h2>
    <p>
      ➨ Reprezintă toate informațiile caracterului dumneavoastră din joc.
    </p>
    <h2>[OOC] - Out Of Character</h2>
    <p>
      ➨ Reprezintă toate informațiile dumneavoastră din viața reală.
    </p>
    <h2>[RP] - Roleplay</h2>
    <p>
      ➨ Reprezintă simularea unei vieți reale.
    </p>
    <h2>[MX] - Mixing</h2>
    <p>
      ➨ Reprezintă amestecarea informațiilor IC cu cele OOC și viceversa.
      <br><br>
      ➨ Vorbitul pe joc este strict IC (poate fi OOC doar cu permisiunea unui membru STAFF). Chat-ul este strict OOC!
      <br><br>
      <b>Exemplu:</b> Emilian este live pe YouTube și joacă pe Bellatrix. Un jucător, în urma vizionării transmisiei, merge la locația lui și îl salută. Jucătorul trebuie
      <br>
      sancționat pentru că a amestecat mediul OOC cu cel IC.
      <br><br>
      <b>Exemplu:</b> În timp ce se află într-un roleplay, un jucător vorbește pe joc că trebuie să plece la baie și că nu va mai fi la tastatură. Jucătorul trebuie sancționat
      <br>
      pentru că a amestecat mediul IC cu cel OOC.
    </p>
    <h2>[PG] - Powergaming</h2>
    <p>
      ➨ Reprezintă atribuirea de acțiuni imposibile în realitate sau roleplay-ul în care o persoană nu primește nicio șansă de răspuns.
      <br><br>
      <b>Exemplu:</b> Într-un schimb de focuri sunteți împușcat în umăr și ulterior folosiți comanda „/cara”. Trebuie să fiți sancționat pentru că v-ați atribuit o acțiune
      <br>
      imposibilă în realitate.
      <br><br>
      <b>Exemplu:</b> Vă aflați într-o bătaie IC și oponentul dumneavoastră trimite mesaje de acest gen: "/me Îi dă un pumn în cap și îl omoară",
      <br>
      "/me Îl împinge și îl face să leșine". Acest jucător trebuie sancționat pentru că nu v-a lăsat nicio șansă de răspuns la roleplay!
    </p>
    <h2>[DM] - Deathmatch</h2>
    <p>
      ➨ Reprezintă omorârea sau lovirea intenționată a unui jucător fără vreun motiv bine întemeiat.
      <br><br>
      <b>Exemplu:</b> Mergeți pe stradă și luați la pumni un individ pentru că este de culoare neagră. Trebuie să fiți sancționat pentru că ați lovit intenționat un jucător fără vreun
      <br>
      motiv bine întemeiat.
    </p>
    <h2>[PK] - Player Kill</h2>
    <p>
      ➨ Reprezintă decesul IC al unui jucător, în urma căruia toate informațiile IC cu privire la acesta din ultimele 3 ore se vor anula.
      <br><br>
      <b>Exemplu:</b> Aflați că un polițist este corupt și peste 20 de minute muriți de foame. În cazul în care vă veți folosi de această informație după respawn, veți fi sancționat.
      <br><br>
      <b>Exemplu:</b> Vă aflați la o dispută între organizații. În urma unui schimb de focuri, omorâți o persoană. În loc să le spuneți camarazilor dumneavoastră doar că ați ucis un inamic, le
      <br>
      comunicați numele victimei, fapt ce determină să nu se respecte PK-ul, deoarece, după respawn, ceilalți jucători îl vor cunoaște pe respectivul ca fiind mort. Prin urmare, veți fi sancționat
      pentru că nu ați respectat PK-ul.
    </p>
    <h2>[CK] - Character Kill</h2>
    <p>
      ➨ Reprezintă decesul IC de mare amploare al unui jucător, în urma căruia caracterul și toate informațiile cu privire la acesta se anulează.
      <br><br>
      ➨ Este nevoie de un Management care să vizioneze acțiunea și să acorde CK-ul dacă decesul dumneavoastră a fost realizat într-un mod cât mai complex și corect!
      <br><br>
      ➨ Se acordă în următoarele situații:
      <br><br>
      　　<u>♦ Trădarea unei facțiuni</u>
      <br><br>
      　　　　<b>Exemplu:</b> Sunteți într-o organizație mafiotă, vindeți poliției informații care pun în pericol gruparea dumneavoastră, sunteți prins de către membri și primiți CK.
      <br><br>
      　　　　<b>Exemplu:</b> Faceți parte din Departamentul de Poliție și, în urma unei percheziții, în loc să depozitați la evidență drogurile găsite, alegeți să le vindeți ilegal. Sunteți prins
      <br>
      　　　　de către superiori și aceștia hotărăsc să vă ofere CK.
      <br><br>
      　　<u>♦ Sinucidere</u>
      <br><br>
      　　　　<b>Exemplu:</b> Ați fost trădat de soție și ați hotărât să vă sinucideți, aruncându-vă de pe cea mai înaltă clădire.
      <br><br>
      　　<u>♦ Act suicidal pentru a scăpa de un roleplay</u>
      <br><br>
      　　　　<b>Exemplu:</b> Sunteți urmărit de poliție și, în loc să vă predați, alegeți să vă aruncați cu vehiculul în mare.
      <br><br>
      　　　　<b>Exemplu:</b> Sunteți capturat de mafioți și, aflându-vă într-un vehicul care circulă cu o viteză periculoasă, alegeți să-l părăsiți din mers pentru a încerca să scăpați.
      <br><br>
      　　<u>♦ Rol de victimă</u>
      <br><br>
      　　　　<b>Exemplu:</b> Ați fost împușcat mortal de mafioți și aceștia v-au lăsat în mijlocul unui drum. Un medic vă găsește și rolează vindecarea dumneavoastră. Dacă jucați rolul de
      <br>
      　　　　victimă și oferiți mesaje de acest gen: "/me Individul nu mai are puls", "/me Individului nu îi mai bate inima", veți primi CK.
      <br><br>
      　　<u>♦ Război între facțiuni</u>
      <br><br>
      　　　　<b>Exemplu:</b> Dețineți o organizație și sunteți în conflict cu o altă grupare. Sunteți de comun acord să faceți război pe CK. În urma acestuia, toți membrii din facțiunea
      <br>
      　　　　pierzătoare vor primi CK, iar aceasta va fi desființată.
      <br><br>
      　　<u>♦ Închisoare pe viață</u>
      <br><br>
      　　　　<b>Exemplu:</b> Sunteți urmărit de poliție pentru diverse infracțiuni și, în urma arestării dumneavoastră, aceștia vă prezintă o sentință de peste 300 de luni pentru faptele comise.
      <br><br>
      　　<u>♦ Alte motive</u>
      <br><br>
      　　　　<b>Exemplu:</b> Un civil v-a distrus căsnicia, v-a lăsat fără loc de muncă și v-a adus într-o stare financiară deplorabilă. Vorbiți cu un Responsabil CK prezentându-i situația, acesta
      <br>
      　　　　vă oferă permisiunea să faceți așa ceva, filmați roleplay-ul în care îl omorâți într-un mod complex, i-l arătați membrului STAFF, pe urmă civilul primește CK!
    </p>
    <h2>[NF] - No Fear</h2>
    <p>
      ➨ Reprezintă alegerea unui jucător de a nu simula frica într-un roleplay în care aceasta este necesară.
      <br><br>
      <b>Exemplu:</b> Sunteți atacat de un mafiot. Acesta are arma îndreptată înspre dumneavoastră și, neavând nicio cale de scăpare, nu efectuați comenzile pe care acesta vi le impune.
      <br>
      Trebuie să fiți sancționat pentru că nu ați simulat frica într-un roleplay în care aceasta este necesară.
      <br><br>
      <b>Exemplu:</b> Treceți insistent cu un vehicul prin fața secției de poliție, nerespectând legislația, pentru a provoca autoritățile să vă urmărească. Trebuie să fiți sancționat pentru că nu
      <br>
      ați simulat frica într-un roleplay în care aceasta este necesară.
      <br><br>
      <b>Exemplu:</b> Sunteți la secție, reținut de niște cadre de poliție și, în loc să vă conformați situației, aveți un comportament indecent cu autoritățile. Trebuie să fiți sancționat
      <br>
      pentru că nu ați simulat frica într-un roleplay în care aceasta este necesară.
    </p>
    <h1>REGULI</h1>
    <br><h3><b>NOTĂ:</b> Pentru fiecare regulă nerespectată, se va acorda numai o variantă din sancțiunile de mai jos, în funcție de gravitatea situației!</h3><br>
    <h2>1. Contul de joc</h2>
    <br>
    <h3>1.1 Numele jucătorului</h3>
    <br>
    <p>
      ➨ Numele trebuie să fie decent, fără semne în exces, fără conotații ascunse!
      <br><br>
      ➨ Alegerea numelor asemănătoare celor existente deja cu scopul de a deranja un jucător sau, şi mai grav, a celor deţinute de membrii STAFF este interzisă!
      <br><br>
      ➨ Este interzis să aveți un nume al cărui număr de caractere este mai mare decât 20!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Kick (avertizare)
      <br>⟶ Ban (120m)
      <br>⟶ Ban (7z)
    </p>
    <h3>1.2 Activitatea contului</h3>
    <br>
    <p>
      ➨ Jucătorii care nu s-au logat în ultimele 60 de zile riscă pierderea proprietăților imobiliare și a afacerilor (dacă există).
      <br><br>
      ➨ În cazul vehiculelor, acestea pot fi pierdute cu un an de inactivitate.
      <br><br>
      ➨ În cazul jucătorilor banați, aceștia pot pierde bunuri în funcție de gravitatea sancțiunii lor.
    </p>
    <h3>1.3 Conturi multiple</h3>
    <br>
    <p>
      ➨ Jucătorii pot avea doar un cont înregistrat pe server.
      <br><br>
      ➨ Este interzis să scăpați de o sancțiune prin utilizarea unui cont diferit!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Ban (permanent)
    </p>
    <h2>2. Interacțiuni sociale</h2>
    <br>
    <h3>2.1 Limbaj</h3>
    <br>
    <p>
      ➨ Este interzis să aduceți jigniri, amenințări sau suferințe la adresa jucătorilor, comunității sau echipei STAFF!
      <br><br>
      ➨ Este interzis să instigați la încălcarea regulamentului!
      <br><br>
      ➨ Este interzis să provocați un conflict sau să luați parte la unul!
      <br><br>
      ➨ Limbajul licențios nu este tolerat, decât în mediul IC, cu limita bunului simț!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Mute (15m - 60m) + Jail (15m)
      <br>⟶ Mute (30m - 120m) + Jail (30m)
      <br>⟶ Ban (15z)
      <br>⟶ Ban (permanent)
    </p>
    <h3>2.2 Reclamă</h3>
    <br>
    <p>
      ➨ Este interzis să faceți orice fel de reclamă!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Ban (15z)
      <br>⟶ Ban (30z)
    </p>
    <h3>2.3 Dovezi</h3>
    <br>
    <p>
      ➨ Este interzis să falsificați orice tip de dovadă!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Ban (30z)
      <br>⟶ Ban (60z)
      <br>⟶ Ban (90z)
    </p>
    <h3>2.4 Comerț</h3>
    <br>
    <p>
      ➨ Este interzis să efectuați plăți OOC pentru bunuri IC și viceversa!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Ban (permanent)
    </p>
    <h3>2.5 Trolling</h3>
    <br>
    <p>
      ➨ Este interzis să stricați în mod intenționat buna desfășurare a comunității!
      <br><br>
      ➨ Este interzis să folosiți un program de redare a sunetelor fără aprobarea unui Management!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Jail (30m)
      <br>⟶ Jail (60m)
      <br>⟶ Jail (180m)
      <br>⟶ Jail (360m)
      <br>⟶ Ban (30z)
      <br>⟶ Ban (permanent)
    </p>
    <h3>2.6 Abuz de bug-uri</h3>
    <br>
    <p>
      ➨ Este interzis să vă folosiți de bug-uri și să nu le raportați la echipa STAFF!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Jail (30m)
      <br>⟶ Jail (60m)
      <br>⟶ Jail (180m)
      <br>⟶ Ban (7z)
      <br>⟶ Ban (30z)
      <br>⟶ Ban (permanent)
    </p>
    <h2>3. Roleplay</h2>
    <br>
    <h3>3.1 Mixing</h3>
    <br>
    <p>
      ➨ Este interzis să faceți Mixing!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Jail (15m)
      <br>⟶ Jail (30m)
      <br>⟶ Jail (60m)
      <br>⟶ Jail (120m)
    </p>
    <h3>3.2 Powergaming</h3>
    <br>
    <p>
      ➨ Este interzis să faceți Powergaming!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Jail (15m)
      <br>⟶ Jail (30m)
      <br>⟶ Jail (60m)
      <br>⟶ Jail (120m)
    </p>
    <h3>3.3 Deathmatch</h3>
    <br>
    <p>
      ➨ Este interzis să faceți Deathmatch!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Jail (60m)
      <br>⟶ Jail (120m)
      <br>⟶ Jail (180m)
      <br>⟶ Jail (240m)
    </p>
    <h3>3.4 Player Kill</h3>
    <br>
    <p>
      ➨ Este interzis să nu respectați un Player Kill!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Jail (15m)
      <br>⟶ Jail (30m)
      <br>⟶ Jail (120m)
      <br>⟶ Jail (180m)
    </p>
    <h3>3.5 No Fear</h3>
    <br>
    <p>
      ➨ Este interzis să faceți No Fear!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Jail (30m)
      <br>⟶ Jail (60m)
      <br>⟶ Jail (120m)
      <br>⟶ Jail (180m)
    </p>
    <h3>3.6 Roleplay scârbos</h3>
    <br>
    <p>
      ➨ Este interzis să faceți roleplay scârbos fără permisiunea celorlalți participanți!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Jail (15m)
      <br>⟶ Jail (30m)
      <br>⟶ Jail (45m)
      <br>⟶ Jail (60m)
    </p>
    <h3>3.7 Conducere auto</h3>
    <br>
    <p>
      ➨ Este interzis să conduceți nebunesc, fără vreun motiv bine întemeiat!
      <br><br>
      ➨ Este interzis să provocați intenționat un accident!
      <br><br>
      ➨ Este interzis să conduceți pe contrasens, fără vreun motiv bine întemeiat!
      <br><br>
      ➨ Este interzis să faceți orice manevră de imobilizare la o viteză de peste 150 de km/h!
      <br><br>
      ➨ Este interzis să aterizați cu elicopterul pe un drum auto, fără vreun motiv bine întemeiat!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Jail (10m)
      <br>⟶ Jail (20m)
      <br>⟶ Jail (30m)
      <br>⟶ Jail (60m)
    </p>
    <h3>3.8 Comă</h3>
    <br>
    <p>
      ➨ Este interzis să faceți vreo acțiune cât timp sunteți în comă!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Jail (10m)
      <br>⟶ Jail (20m)
      <br>⟶ Jail (30m)
      <br>⟶ Jail (60m)
    </p>
    <h3>3.9 Înșelăciune</h3>
    <br>
    <p>
      ➨ Este interzis să înșelați pe cineva dacă suma totală de câștig este mai mare decât 1000 lei!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Jail (30m)
      <br>⟶ Jail (60m)
      <br>⟶ Jail (120m)
      <br>⟶ Ban (7z)
      <br>⟶ Ban (15z)
    </p>
    <h3>3.10 Corupție</h3>
    <br>
    <p>
      ➨ Este interzis ca liderii de la facțiunile de stat să fie corupți!
      <br><br>
      ➨ Este interzis ca membrii unei facțiuni de stat să ofere bunurile acesteia!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Jail (30m)
      <br>⟶ Jail (60m)
      <br>⟶ Jail (120m)
      <br>⟶ Ban (7z)
      <br>⟶ Ban (15z)
      <br>⟶ Ban (30z)
    </p>
    <h3>3.11 Raid-uri</h3>
    <br>
    <p>
      ➨ Este interzis ca o organizație, alta decât cele oficiale, să facă un raid!
      <br><br>
      ➨ Pentru a se da raid la o facțiune este necesar ca minim 10 membri din aceasta să fie în joc!
      <br><br>
      ➨ Fiecare facțiune oficială are permisiunea de a da un singur raid pe săptămână!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Jail (30m)
      <br>⟶ Jail (60m)
      <br>⟶ Jail (120m)
    </p>
    <h3>3.12 Instituție</h3>
    <br>
    <p>
      ➨ Este interzis să faceți activități periculoase într-o instituție (curtea de apel, secția de poliție, spitalul etc.)!
      <br><br>
      ➨ Este interzis să se fure bunuri din depozitele facțiunilor legale!
      <br><br>
      ➨ Este interzis să se facă roleplay că se șterge baza de date a unei instituții!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Jail (30m)
      <br>⟶ Jail (60m)
      <br>⟶ Jail (120m)
    </p>
    <h3>3.13 Loc de muncă</h3>
    <br>
    <p>
      ➨ Este interzis să fiți la un loc de muncă cu uniformă și să jefuiți sau răpiți!
      <br><br>
      ➨ Este interzis să folosiți un vehicul de la locul de muncă pentru scopurile personale!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Jail (15m)
      <br>⟶ Jail (30m)
      <br>⟶ Jail (60m)
      <br>⟶ Jail (120m)
    </p>
    <h2>4. Jafuri persoane</h2>
    <br>
    <h3>4.1 Bunuri</h3>
    <br>
    <p>
      ➨ Este interzis să jefuiți bunurile unei persoane, iar pe urmă să o omorâți și viceversa!
      <br><br>
      ➨ Regula nu se aplică în cazul în care victima nu respectă comenzile atacatorului.
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Jail (15m)
      <br>⟶ Jail (30m)
      <br>⟶ Jail (60m)
      <br>⟶ Jail (120m)
    </p>
    <h3>4.2 Abuz jaf</h3>
    <br>
    <p>
      ➨ Este interzis să tâlhăriți aceeași persoană, decât după 3 ore de la ultimul jaf asupra acesteia!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Jail (20m)
      <br>⟶ Jail (45m)
      <br>⟶ Jail (60m)
    </p>
    <h2>5. Jafuri clădiri</h2>
    <br>
    <h3>5.1 Ostatic</h3>
    <br>
    <p>
      ➨ Este interzis să aveți un ostatic fals!
      <br><br>
      <b>SANCȚIUNI:</b>
      <br>⟶ Jail (15m)
      <br>⟶ Jail (30m)
      <br>⟶ Jail (45m)
      <br>⟶ Jail (60m)
    </p>
    <br><br>
    `
  }
  
  for (const o in prs) {
    CreateOption(o);
  }
  
  selectOption("Regulament");
  
  function CreateOption(opt)
  {
    var navOptions = document.querySelector(".navigation")
    let Option = document.createElement("div")
    Option.classList.add('nav-option');
    Option.onclick = function(event) {
      selectOption(opt);
    }
    Option.innerText = opt;
    navOptions.appendChild(Option)
  }
  
  function selectOption(ex) {
    var doc = document.querySelector(".details")
    doc.innerHTML = prs[ex]
  }
  
  $(function () {
    function display(bool) {
        if (bool) {
            $(".container").fadeIn();
        } else {
            $(".container").fadeOut();
        }
    }
  
    display(false);
  
    window.addEventListener('message', function(event) {
        var item = event.data;
        if (item.type === "ui") {
            if (item.status == true) {
                display(true)
            } else {
                display(false)
            }
        }
    })
  
    document.onkeyup = function(data){
      if(data.which == 27){
        $.post('https://info/exit', JSON.stringify({}));
        return
      }
    }
  
  })
  
